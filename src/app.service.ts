    ...
    try {
      const collectionsListRaw = await browserPage.evaluate(() => {
        const script = Array.from(document.querySelectorAll('script')).find(
          (el) => el.innerText.includes('Alpine.store("collections",'),
        );
        if (script) {
  ...
  async getContractAddressList(browserPage) {
    try {
      ...
      const firstPageData = await this.getBlurTopCollectionsOneWeekList(
        browserPage,
        '%7B%22sort%22%3A%22VOLUME_ONE_WEEK%22%2C%22order%22%3A%22DESC%22%7D',
      );
      ...
      if (firstPage.isEnd) {
        ...
        while (!isLoopEnd) {
          ...
          const { contracts, isEnd } =
            await this.addCollectionsUntilAmountIsOne(
  ...
  async addCollectionsUntilAmountIsOne(collections) {
    ...
    for (const item of collections) {
      try {
        ...
        console.error(
          `An error occurred in addCollectionsUntilAmountIsOne: ${error}`,
        );
      }
    }
